...
pub(crate) struct ConstraintSource {
	...
	/// The indices of the originating constraint items (e.g., the positions of
	/// the constraint items in the FlatZinc instance).
...
impl ProofState {
	...
	/// the source and the indices of the originating constraint items, if any.
	pub(crate) fn resolve_hint(&self) -> (&str, &[u32]) {
		match &self.next_hint {
			Some(ProofSource::Constraint(src)) => (src.name, &src.ids),
			Some(ProofSource::Propagator(p)) => match &self.propagator_source[p.index()] {
				Some(src) => (src.name, &src.ids),
				None => ("", &[]),
			},
			Some(ProofSource::Rule(name)) => (name, &[]),
			None => ("", &[]),
		}
	}
}
